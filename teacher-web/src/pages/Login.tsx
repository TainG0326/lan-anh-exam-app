...
export default function Login() {
  ...
  // ====== 2FA OTP INPUT COMPONENT ======
  // Single OTP box - consistent styling for both setup and verify modes
  const renderOtpBox = (
    index: number,
    mode: 'setup' | 'verify',
    value: string,
    ...
    const refs = mode === 'setup' ? otpSetupRefs : otpVerifyRefs;
    const prefix = mode === 'setup' ? 'otp-setup' : 'otp-verify';
    const filled = !!value;
      ...
      <input
        ...
        onChange={(e) => {
          ...
          if (raw.length > 1) {
            ...
            onChange(newOtp);
            setTimeout(() => {
              const last = document.getElementById(`${prefix}-${Math.min(newOtp.length, 5)}`);
              last?.focus();
            }, 0);
            if (newOtp.length === 6) {
              const form = document.getElementById(`otp-form-${mode}`);
              if (form) setTimeout(() => form.dispatchEvent(new Event('submit', { bubbles: true })), 150);
            }
          } else {
            const newOtp = value.slice(0, index) + raw + value.slice(index + 1);
            onChange(newOtp);
            if (raw) {
              setTimeout(() => {
                if (index < 5) {
        ...
        onKeyDown={(e) => {
          if (e.key === 'Backspace') {
            ...
            onChange(newOtp);
            setTimeout(() => {
              const target = document.getElementById(`${prefix}-${Math.max(0, index - (value[index] ? 0 : 1))}`);
              target?.focus();
            }, 0);
          }
          if (e.key === 'ArrowLeft') {
        ...
        id={`${prefix}-${index}`}
        className={`
          w-11 h-12 text-center text-lg font-bold tracking-widest transition-all duration-200 border-2 rounded-xl select-none
          ${filled
            ? 'bg-primary/10 border-primary text-primary'
            : 'bg-white border-gray-200 text-gray-800 hover:border-gray-300'
          }
          focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary focus:bg-primary/10
          cursor-text
        `}
      />
    );
  ...
  const renderMobileOtp = (
    ...
    <div>
      <input
        ...
        className="w-full px-5 py-4 text-center text-2xl tracking-[0.5em] font-mono font-bold border-2 border-gray-200 bg-white text-gray-800 focus:border-primary focus:bg-primary/5 focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all"
        ...
        {[...Array(6)].map((_, i) => (
          <div
            ...
            className={`w-2 h-2 rounded-full transition-all duration-200 ${
              i < value.length ? 'bg-primary' : 'bg-gray-300'
            }`}
          />
        ))}
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-[0_8px_40px_rgba(0,0,0,0.25)] overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row">
          ...
          <div className="md:w-[54%] px-10 py-8">
            ...
            {/* OTP Form */}
            <form id="otp-form-setup" onSubmit={handleVerifyOTP} className="space-y-5">
              {/* Desktop OTP */}
              <div className="hidden sm:flex items-center justify-center gap-2">
                {[...Array(6)].map((_, i) =>
                  renderOtpBox(i, 'setup', otp, setOtp, () => handleVerifyOTP({ preventDefault: () => {} } as React.FormEvent))
                )}
              </div>

              {/* Mobile OTP */}
              <div className="sm:hidden">
                {renderMobileOtp('setup', otp, setOtp)}
              </div>
    ...
    >
      <div className="bg-white rounded-2xl shadow-[0_8px_40px_rgba(0,0,0,0.25)] overflow-hidden">
        ...
        <div className="px-8 pb-8 pt-6">
          <form id="otp-form-verify" onSubmit={handleVerifyOTP} className="space-y-5">
            {/* Email hint */}
            ...
            <div className="hidden sm:flex items-center justify-center gap-2">
              {[...Array(6)].map((_, i) =>
                renderOtpBox(i, 'verify', otp, setOtp, () => handleVerifyOTP({ preventDefault: () => {} } as React.FormEvent))
              )}
            </div>

            {/* Mobile OTP */}
            <div className="sm:hidden">
              {renderMobileOtp('verify', otp, setOtp)}
            </div>
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden">
      ...
      {/* Content */}
      {requires2FA && requiresSetup ? otpSetupContent : null}
      {requires2FA && !requiresSetup ? otpVerifyContent : null}
      {!requires2FA ? loginContent : null}
    </div>
  );
}
